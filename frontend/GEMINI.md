...
- If prompt mentions to use attached images for taking design inspiration then strictly copy styling, colors, typography, layout of the provided attached images.
- If prompt mentions to create a page or app like this or like this attached image then **ALWAYS** create a page like attached images exactly copying content, layout, styling, colors, typography etc. this is strict requirement.
...
- On `AUTH_SUCCESS`: store tokens in localStorage and refresh window using `window.location.reload()`
