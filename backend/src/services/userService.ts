 ...
 * Check and reset session count for a user if a week has passed

 */

export async function checkAndResetSessionCount(userId: string): Promise<User> {
    ...
    if (now > user.weeklyResetAt) {
        ...
        // Reset count and set next reset date
